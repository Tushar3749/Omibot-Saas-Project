...
import {
  ...
  Search, ShoppingBag, Zap, MapPin, Heart, MessageSquare, Package,
} from 'lucide-react'
...
const TABS = [
  ...
  { id: 'integrations', label: 'ইন্টিগ্রেশন',     icon: Package },
...
function ComingSoon({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative" title="এই feature শীঘ্রই চালু হবে">
...
function TabComingSoon({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative min-h-96">
      <div style={{ opacity: 0.18, pointerEvents: 'none', userSelect: 'none', filter: 'blur(1.5px)' }}>
        {children}
      </div>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        ...
        >
          ...
          <p className="text-sm" style={{ color: 'var(--c-muted)' }}>এই feature শীঘ্রই চালু হবে</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="max-w-3xl space-y-5">
      ...
      {activeTab === 'identity' && (
        <div className="space-y-4">
          <SectionCard icon={Bot} title="Bot পরিচয়" subtitle="দোকান, Bot-এর নাম, ভাষা ও ব্যক্তিত্ব">
      ...
      {activeTab === 'orders' && (
        <div className="space-y-4">
          <SectionCard icon={ShoppingBag} title="Order Management" subtitle="AI অর্ডার গ্রহণের নিয়ম নির্ধারণ করুন">
            <div className="grid grid-cols-2 gap-4">
              <NumField
                label="রিটার্ন সময়সীমা (দিন)"
                value={String(config.return_window_days ?? 7)}
                onChange={v => setConfig(c => ({ ...c, return_window_days: parseInt(v) || 7 }))}
                min={0}
                suffix="দিন"
              />
            </div>
      ...
      {activeTab === 'ai' && (
        <div className="space-y-4">
          <SectionCard icon={Zap} title="AI আচরণ" subtitle="Bot-এর স্বয়ংক্রিয় প্রতিক্রিয়া কনফিগার করুন">
            <div className="grid grid-cols-1 gap-3">
              <Toggle
                checked={Boolean(config.product_image_auto_send)}
      ...
      {/* Save button (bottom) — not shown for local tab which has its own save */}
